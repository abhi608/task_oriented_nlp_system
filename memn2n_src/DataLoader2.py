...
from data_utils2 import load_candidates, load_dialog_task, vectorize_candidates, vectorize_data
        ...
        if self.nn:
            if self.train == 0:
                self.S, self.Q, self.A = vectorize_data(
                    self.trainData, self.word_idx, self.sentence_size, self.batch_size, self.n_cand, self.memory_size, nn=self.nn, max_story_size=self.max_story_size)
            elif self.train == 1:
                self.S, self.Q, self.A = vectorize_data(
                    self.valData, self.word_idx, self.sentence_size, self.batch_size, self.n_cand, self.memory_size, nn=self.nn, max_story_size=self.max_story_size)
            elif self.train == 2:
                self.S, self.Q, self.A = vectorize_data(
                    self.testData, self.word_idx, self.sentence_size, self.batch_size, self.n_cand, self.memory_size, nn=self.nn, max_story_size=self.max_story_size)
        else:
            if self.train == 0:
                self.S, self.Q, self.A = vectorize_data(
                    self.trainData, self.word_idx, self.sentence_size, self.batch_size, self.n_cand, self.memory_size, nn=self.nn, max_story_size=self.max_story_size)
